 ...
 */
                            ...
                            TaskHandle_t * const pxCreatedTask ) 
    {
        ...
        RT_DEBUG_LOG(RT_DEBUG_THREAD, ("new task: %s\r\n", pcName));

        tid = rt_thread_create(pcName, pxTaskCode, pvParameters, usStackDepth, (rt_uint8_t)rtt_priority, 50);
        if (tid != RT_NULL)
        {
            if( ( void * ) pxCreatedTask != NULL )
            {
                *pxCreatedTask = ( TaskHandle_t ) tid;//直接传回线程tid 可能有bug！！！
            }
 ...
 * set to 1 in FreeRTOSConfig.h for pcTaskGetName() to be available.
 *
 * \ingroup TaskUtils
 */
