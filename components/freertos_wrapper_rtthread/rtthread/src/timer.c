...
void rt_timer_check(void)
{
    ...
    while (!rt_list_isempty(&rt_timer_list[RT_TIMER_SKIP_LIST_LEVEL - 1]))
    {
         ...
         * tick max.
         */
        RT_DEBUG_LOG(RT_DEBUG_TIMER, ("tick: %d timeout_tick:%d\n", current_tick, t->timeout_tick));//by jz
...
static void rt_thread_timer_entry(void *parameter)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_TIMER, ("timer thread running ...\n"));//by jz
    ...
    while (1)
    {
        ...
        if (next_timeout == RT_TICK_MAX)
        {
            ...
            RT_DEBUG_LOG(RT_DEBUG_TIMER, ("start schedule\n"));//by jz
            rt_schedule();
        }
        else
        {
