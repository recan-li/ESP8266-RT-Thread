                                ...
                                rt_uint32_t       tick)
{
    ...
    RT_DEBUG_MORE("task: %s, sp: 0x%08x", thread->name, thread->sp);
                        ...
                        rt_uint32_t       tick)
{
    ...
    /* init thread object */
    RT_DEBUG_MORE("thread: %p %s", thread, name);
...
rt_err_t rt_thread_startup(rt_thread_t thread)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_THREAD, ("startup task: %p %p %s (stack: 0x%08x ~ 0x%08x) %d bytes\r\n", \
                    thread, thread->sp, thread->name, thread->stack_addr, \
                    thread->stack_addr + thread->stack_size - 1, thread->stack_size));
                             ...
                             rt_uint32_t tick)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_THREAD, ("create new task: %s\n", name));
...
void rt_thread_timeout(void *parameter)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_THREAD, ("thread timeout:%s with priority:%d\n", thread->name, thread->init_priority));//by jz
